...
export const AnimatedTestimonials = ({
  ...
  // Fixed rotation values for consistency between server and client
  const rotationValues = [-8, -6, -4, -2, 0, 2, 4, 6, 8];
  ...
  // Get a deterministic rotation value based on index
  const getRotateY = (index: number) => {
    return rotationValues[index % rotationValues.length];
  };

  // Only render animations after component has mounted on client side
  if (!mounted) {
    return (
          ...
          <div className="flex flex-col justify-between py-4">
            ...
            <div className="flex gap-4 pt-12 md:pt-0">
              <button className="group/button flex h-7 w-7 items-center justify-center rounded-full bg-gray-100 dark:bg-neutral-800">
                <ArrowLeftIcon className="h-5 w-5 text-black dark:text-neutral-400" />
              </button>
              <button className="group/button flex h-7 w-7 items-center justify-center rounded-full bg-gray-100 dark:bg-neutral-800">
                <ArrowRightIcon className="h-5 w-5 text-black dark:text-neutral-400" />
              </button>
            </div>
          </div>
        </div>
